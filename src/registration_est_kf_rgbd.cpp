...
struct RegistrationResult registration_est_kf_rgbd(PointCloud ptcldMoving, PointCloud ptcldFixed) {
    ...
    tolerance << 0.001 , .009; // Tolerance = [.001 .009]
    ...
    for (int i = 1; i <= min(MAX_ITERATIONS, sizePtcldMoving / WINDOW_SIZE); i++) {
        ...
        MatrixXd targets(3, WINDOW_SIZE);
        ...
        // from last iteration 
        struct KdResult searchResult = kd_search(targets, WINDOW_SIZE, cloudTree,
        ...
        // Truncate the WINDOW_SIZE according to INLIER_RATIO
